...
const BookForm = props => {
  ...
  const handleChange = e => {
    ...
    cleanError();
    document.querySelector(book ? '.errorUpdate' : '.errorForm').classList.remove('display-error');
  };
  const handleSubmit = async e => {
    ...
    if (state.category !== 'Category' && state.title.length > 0) {
      ...
      if (!book) {
        ...
        document.querySelector(book ? 'editBook' : 'createBook').value = '';
      }
      form.classList.remove('display-error');
    } else {
      form.classList += ' display-error';
      form.innerHTML = 'Please fill the form correctly';
    }
  };
  return (
    <div>
